...
async def keep_awake_task():
    ...
    while True:
        await asyncio.sleep(600) # Every 10 mins ping itself
        try:
...
"
    ...
    try:
        ...
        if editor_id == owner_id:
            # Agar SIZ o'zgartirsangiz -> Uning bot lichkasiga boradi
            target_chat = message.chat.id
        else:
            # Agar U o'zgartirsa -> Sizning bot lichkangizga keladi
...
'
                ...
                try:
                    if uid == owner_id:
                        # Agar SIZ o'chirsangiz -> Uning bot lichkasiga boradi
                        target_chat = chat_id
                    else:
                        # Agar U o'chirsa -> Sizning bot lichkangizga keladi
                        target_chat = owner_id
...
async def adm_edit_start_init(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text("Yangi Start xabarini yuboring (Premium emojilar saqlanadi):")

@dp.message(AdminStates.edit_start_msg)
async def adm_edit_start_save(message: Message, state: FSMContext):
    # HTML format bilan saqlash (Premium Emojilar uchun)
...
async def adm_broadcast_proc(message: Message, state: FSMContext):
    ...
    for row in rows:
        uid = row[0]
        try:
            # copy_message barcha tugma, format va emojilarni aniq nusxalaydi
            await bot.copy_message(chat_id=uid, from_chat_id=message.chat.id, message_id=message.message_id)
            succ += 1
        except Exception:
            err += 1
        await asyncio.sleep(0.05) # Spam limitdan qochish uchun
