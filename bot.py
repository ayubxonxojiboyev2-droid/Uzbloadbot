                    ...
                    for user_id, exp_str in rows:
                        ...
                        if datetime.timedelta(0) < time_left <= datetime.timedelta(days=1):
                            ...
                            try:
                                ...
                                logging.error(f"Failed to send sub warning to {user_id}: {e}")
        except Exception as e:
...
async def start_cmd(message: Message, state: FSMContext):
    ...
    await message.answer(start_text, reply_markup=start_inline_kb())

@dp.callback_query(F.data == "back_to_start")
async def back_to_start_call(call: CallbackQuery, state: FSMContext):
    await state.clear()
    start_text = await get_setting("start_message")
    await call.message.edit_text(start_text, reply_markup=start_inline_kb())
...
async def track_business_message(message: Message):
    try:
        ...
        if not await has_active_sub(conn.user_chat_id):
            return
    except Exception:
        return
...
async def track_edited_business_message(message: Message):
    try:
        ...
        if not await has_active_sub(owner_id):
            return
    except Exception:
        return
    ...
    try:
        ...
        if editor_id == owner_id:
            # Agar siz o'z xabaringizni tahrirlasangiz, bot xabarni BUSINESS CHAT ga (ikkinchi odamga) yuboradi
            ...
            # Agar boshqa odam tahrirlasa, bot sizning PM lichkangizga (bildirmasdan) yuboradi
            await bot.send_message(
                chat_id=owner_id, 
                text=notify_text, 
                parse_mode="HTML"
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        for msg_id in event.message_ids:
            ...
            if row:
                ...
                try:
                    if uid == owner_id:
                        # Agar siz o'chirsangiz, biznes chatga (ikkinchi odamga) yuboradi
                        target_chat = chat_id
                        b_conn_id = event.business_connection_id
                        if file_id:
                            if c_type == ContentType.PHOTO:
                                await bot.send_photo(target_chat, photo=file_id, caption=report, business_connection_id=b_conn_id, parse_mode="HTML")
                            elif c_type == ContentType.VIDEO:
                                await bot.send_video(target_chat, video=file_id, caption=report, business_connection_id=b_conn_id, parse_mode="HTML")
                            elif c_type == ContentType.VOICE:
                                await bot.send_voice(target_chat, voice=file_id, caption=report, business_connection_id=b_conn_id, parse_mode="HTML")
                        else:
                            await bot.send_message(target_chat, report, business_connection_id=b_conn_id, parse_mode="HTML")
                    else:
                        # Agar boshqa odam o'chirsa, sizga PM ga yuboradi
