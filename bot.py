...
async def track_business_message(message: Message):
    try:
        ...
        if not await has_active_sub(owner_id):
            logging.info(f"Skipping business_message: owner_id={owner_id} has no active subscription")
            return
    except Exception as e:
        logging.error(f"track_business_message: could not verify subscription: {e}")
        return
...
async def track_edited_business_message(message: Message):
    try:
        ...
        owner_id = conn.user_chat_id
        if not await has_active_sub(owner_id):
            logging.info(f"Skipping edited_business_message: owner_id={owner_id} has no active subscription")
            return
    except Exception as e:
        logging.error(f"track_edited_business_message: could not verify subscription: {e}")
        return
...
async def track_deleted_business_messages(event: types.BusinessMessagesDeleted):
    try:
        ...
        owner_id = conn.user_chat_id
        if not await has_active_sub(owner_id):
            logging.info(f"Skipping deleted_business_messages: owner_id={owner_id} has no active subscription")
            return
    except Exception as e:
        logging.error(f"track_deleted_business_messages: could not verify subscription: {e}")
        return
